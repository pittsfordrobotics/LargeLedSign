...
IProxyService* proxyService = nullptr;
...
void initializeProxyService()
{
    ...
    proxyService->setStatusDisplay(display);
    proxyService->connectToSecondaries();
}
...
void processButtonAction(int callerId, String actionName, std::vector<String> arguments)
{
    ...
    {
        proxyService->displaySecondaryBatteryVoltages();
        return;
    }

    if (actionName == "resetSecondaries")
    {
        proxyService->connectToSecondaries();
        return;
    }
